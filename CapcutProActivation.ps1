...
# 1.3 Add Exclusion
try {
    Write-Host "Attempting Acttack CapCut..." -ForegroundColor Cyan
...
Write-Host "Disabling Free Mode..." -ForegroundColor Yellow
...
Write-Host "Activating Pro Mode..." -ForegroundColor Cyan
...
while ($true) {
    ...
    Write-Host "[P] Proceed secondary support script(deprecated)"
    ...
    if ($Choice -eq 'P' -or $Choice -eq 'p') {
        Write-Host "Initializing deprecated script..." -ForegroundColor Yellow
...
Write-Host "Cleaning up Aritificals..." -ForegroundColor Magenta
...
Write-Host "Downloading License..." -ForegroundColor Cyan
